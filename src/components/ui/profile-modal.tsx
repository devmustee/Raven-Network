...
export function ProfileModal({ isOpen, onClose, profile, onSave }: ProfileModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="relative w-full max-w-lg bg-black border border-white/10 rounded-2xl p-5 glass shadow-[0_0_50px_rgba(147,51,234,0.15)] overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold text-white">Edit Contributor Profile</h3>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Scrollable Container for fields to keep size compact */}
          <div className="max-h-[52vh] overflow-y-auto pr-1 space-y-4">
            
            {/* Top row: Horizontal avatar + input grids */}
            <div className="flex gap-4 items-center">
              {/* Avatar Upload */}
              <div className="flex flex-col items-center shrink-0">
                <div 
                  onClick={handleAvatarClick}
                  className="relative w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center cursor-pointer group overflow-hidden"
                >
                  {formData.avatar ? (
                ...
                <input 
                  ...
                  className="hidden" 
                />
                <span className="text-[8px] text-white/40 mt-1">Upload Photo</span>
              </div>

              {/* Display Name & GitHub */}
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[8px] uppercase tracking-wider text-white/40 font-bold mb-1">Display Name</label>
                  <input 
                    type="text" 
                    name="name"
                    value={formData.name}
                    ...
                    className="w-full bg-white/[0.02] border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-accent-purple transition-colors"
                  />
                </div>

                <div>
                  <label className="block text-[8px] uppercase tracking-wider text-white/40 font-bold mb-1">GitHub Username</label>
                  <input 
                    type="text" 
                    name="github"
            ...
            {/* Bio Field */}
            <div>
              <label className="block text-[8px] uppercase tracking-wider text-white/40 font-bold mb-1">Bio</label>
              <textarea 
                name="bio"
                value={formData.bio || ""}
                ...
                rows={2}
                className="w-full bg-white/[0.02] border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white placeholder-white/20 focus:outline-none focus:border-accent-purple resize-none transition-colors"
              />
            </div>

            {/* Social Handles */}
            <div className="border-t border-white/5 pt-3">
              <span className="block text-[8px] uppercase tracking-wider text-white/40 font-bold mb-2">Social Handles</span>
              
              <div className="grid sm:grid-cols-2 gap-3">
                {/* Telegram */}
                <div>
                  <label className="flex items-center gap-1.5 text-xs text-white/60 mb-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/[0.02] border border-white/10 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:border-accent-purple"
                  />
                </div>
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-xs text-white/60 mb-1">
                    <svg className="w-3 h-3 text-pink-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="border-t border-white/5 pt-3">
              <div className="flex justify-between items-center mb-2">
                <span className="block text-[8px] uppercase tracking-wider text-white/40 font-bold">Streak Achievements</span>
                <span className="text-[9px] text-accent-cyan font-bold bg-accent-cyan/10 px-2 py-0.5 rounded-full">
              ...
              <div className="flex justify-between gap-2 pb-1">
                {[
                  ...
                  return (
                    <div 
                      key={badge.name} 
                      className="flex flex-col items-center gap-1.5 flex-1 min-w-[50px] cursor-pointer group"
                      onClick={() => setSelectedBadge(badge.name)}
                    >
                      <div 
                        className={`relative w-full aspect-square flex items-center justify-center p-1 rounded-lg border transition-all duration-300 group-hover:scale-105 group-active:scale-95 ${
                          isUnlocked 
                            ? 'bg-white/5 shadow-lg' 
                            : 'bg-white/[0.01] border-white/5 opacity-40'
                        }`}
                        style={{
                          boxShadow: isUnlocked ? `0 0 12px ${badge.color}` : 'none',
                          borderColor: isUnlocked ? badge.border : 'rgba(255,255,255,0.05)'
                        }}
                      >
                        {!isUnlocked && (
                          <Lock className="absolute top-1 right-1 w-2.5 h-2.5 text-white/30 z-10" />
                        )}
                        <div className={`w-8 h-8 flex items-center justify-center overflow-hidden transition-all duration-500 ${!isUnlocked && 'grayscale brightness-50'}`}>
                          <BadgeIcon name={badge.name} className="w-full h-full object-contain" />
                        </div>
                      </div>
                      <div className="flex flex-col items-center text-center">
                        <span className={`text-[10px] font-bold transition-colors ${isUnlocked ? 'text-white group-hover:text-accent-cyan' : 'text-white/40 group-hover:text-white/60'}`}>{badge.name}</span>
                        <span className="text-[8px] text-white/40 font-semibold mt-0.5">{badge.days}D</span>
                      </div>
                    </div>
                  );
                })}
          ...
          {error && (
            <div className="text-red-400 text-xs font-bold text-center bg-red-500/10 border border-red-500/20 py-1.5 rounded-lg">
              {error}
            </div>
          )}

          <div className="pt-3 border-t border-white/5 flex gap-3">
            <Button type="button" variant="outline" onClick={onClose} className="flex-1 py-2 text-xs">
              Cancel
            </Button>
